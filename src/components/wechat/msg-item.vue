<template>
    ...
    <li :class="{'item-hide':deleteMsg}">
        ...
        <router-link :to="{ path: '/wechat/dialogue', query: { mid: item.mid,name:item.group_name||($store.getters.get_user_list(item.user).remark||$store.getters.get_user_list(item.user).nickname),group_num:item.user.length}}" tag="div" class="list-info" v-swiper v-on:click.native="toggleMsgRead($event,'enter')">
            <div class="header-box">
                ...
                <div class="header" :class="[item.type=='group'?'multi-header':'']">
                    <img v-for="userInfo in this.$store.getters.get_user_list(item.user)" :src="userInfo.headerUrl">
                </div>
            </div>

            <div class="desc-box">
                <!--使用过滤器 fmtDate 格式化时间-->
                <div class="desc-time">{{item.msg[item.msg.length-1].report_time | fmtDate('hh:ss')}}</div>
                <div class="desc-author" v-if="item.type=='group'">{{item.group_name}}</div>
                <!--如果没有备注好友，则显示微信昵称-->
                <div class="desc-author" v-else>{{this.$store.getters.get_user_list(item.user)[0].remark||this.$store.getters.get_user_list(item.user)[0].nickname}}</div>
                <div class="desc-msg">
        ...
        <div class="operate-box">
            <!-- 关闭这里的功能，并且将左滑右滑的最小感应距离改为1000，原先是20，那么就不会触发左滑右滑停留 -->
                ...
                bind: function(element, binding) {
                    ...
                    element.addEventListener('touchmove', function(e) {
                        ...
                        if (distanceX < 0) { //右滑
                            ...
                            if (Math.abs(distanceX) >= Math.abs(distanceY)) {
                                if (distanceX < 156 && distanceX > 2000) {
                                    e.preventDefault()
